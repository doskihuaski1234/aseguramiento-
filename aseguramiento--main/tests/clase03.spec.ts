...
test.describe('Laboratorio 03 - Locator por texto', () => {
  ...
  test('Locators encadenados: precio dentro de una tarjeta', async ({ page }) => {
    ...
    // Buscar solo dentro de la primera tarjeta
    const primeraTarjeta = page.locator('.card').first();

    // Buscar el precio dentro de esa tarjeta
    ...
    // Evidencia
    await page.screenshot({
      path: 'evidencias/laboratorio03-precio.png',
      fullPage: true
    });
  ...
  test('Verificar que NO existe un elemento (negación)', async ({ page }) => {
    ...
    // Evidencia
    await page.screenshot({
      path: 'evidencias/laboratorio03-negacion.png',
      fullPage: true
    });
